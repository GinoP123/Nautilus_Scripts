...
command = f"sudo lsof -i :{settings.port} -P -n | grep ESTABLISHED"
...
port_forward_command = f"kubectl port-forward {pod_name} {settings.port}:{settings.port}"
...
kernel_api = f'http://localhost:{settings.port}/api/kernels'
